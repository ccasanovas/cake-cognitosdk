...
trait ImportApiUsersTrait
{
    ...
    protected function _import($headers = [])
    {
        ...
        if(empty($csv_data)){
            $this->Flash->warning(__d('Ccasanovas/CognitoSDK', 'Nothing to import.'));
            return;
        }
        ...
        if($data_rows > $max_rows){
            $this->Flash->error(__d('Ccasanovas/CognitoSDK',
        ...
        if($result){
            $this->Flash->success(__d('Ccasanovas/CognitoSDK', '{0} API Users imported successfully', count($result)));
        }else{
            $this->Flash->error(__d('Ccasanovas/CognitoSDK', 'An error occurred while trying to import the API Users. Please try again.'));
        }
