<div class="dropdown">
    ...
    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="contextualMenuApiUser">
        ...
        <?php if($api_user->aws_cognito_status['code'] === 'CONFIRMED'): ?>
           <li>
            <?= $this->Form->postLink(
                __d('Ccasanovas/CognitoSDK', 'Reset Password'),
                ['action' => 'resetPassword', $api_user->id],
                [
                    'confirm' => __d('Ccasanovas/CognitoSDK', 'Are you sure you want to reset the password of # {0}?', $api_user->aws_cognito_username)
                ]
            ) ?>
            </li>
        <?php else: ?>
            <li class="disabled">
                <a href="#"><?= __d('Ccasanovas/CognitoSDK', 'Reset Password') ?></a>
            </li>
        <?php endif; ?>
        <?php if($api_user->aws_cognito_status['code'] === 'FORCE_CHANGE_PASSWORD'): ?>
            <li><?= $this->Html->link(
                __d('Ccasanovas/CognitoSDK', 'Resend Invitation Email'),
                ['action' => 'resendInvitationEmail', $api_user->id]
            ) ?></li>
        <?php else: ?>
            <li class="disabled">
                <a href="#"><?= __d('Ccasanovas/CognitoSDK', 'Resend Invitation Email') ?></a>
            </li>
        <?php endif; ?>
        <li class="divider"></li>
        <li>
