<?php
namespace Ccasanovas\CognitoSDK\Test\TestCase\Model\Table;
...
class ApiUsersTableTest extends TestCase
{
	...
	{
		parent::setUp();
		$this->ApiUsers = TableRegistry::get('Ccasanovas/CognitoSDK.ApiUsers');
	}
	...
	public function testGetRolesFailedEmptySettings()
	{
		...
		$this->expectExceptionMessage(__d('Ccasanovas/CognitoSDK', 'ApiUsers.roles setting is invalid'));
	...
	{
		$roles = ['user', 'admin'];
		$this->expectExceptionMessage(__d('Ccasanovas/CognitoSDK', 'The ApiUsers.roles array should be entirely associative'));
		Configure::write('ApiUsers.roles', $roles);
		$this->ApiUsers->getRoles();
	}
